...
if [ -n "$HOSTNAME" ]; then
    sed -i "s/HOSTNAME=your.host.name/HOSTNAME=$HOSTNAME/g" docker-compose.yml
fi

read -p "What is your SSL contact email? " SSL_CONTACT_EMAIL
if [ -n "$SSL_CONTACT_EMAIL" ]; then
    sed -i "s/SSL_CONTACT_EMAIL=YourEmail@gmail.com/SSL_CONTACT_EMAIL=$SSL_CONTACT_EMAIL/g" docker-compose.yml
fi
...
if [[ "$SMTP_EMAIL_NOTIFICATIONS_ENABLED" =~ [Yy] ]]; then
    ...
    if [ -n "$SMTP_HOST" ]; then
        sed -i "s/SMTP_HOST=smtp.gmail.com/SMTP_HOST=$SMTP_HOST/g" docker-compose.yml
    fi

    read -p "What is your SMTP port (default 465)? " SMTP_PORT
    if [ -n "$SMTP_PORT" ]; then
        sed -i "s/SMTP_PORT=465/SMTP_PORT=$SMTP_PORT/g" docker-compose.yml
    fi
    ...
    if [ -n "$SMTP_USERNAME" ]; then
        sed -i "s/SMTP_USERNAME=YourEmail@gmail.com/SMTP_USERNAME=$SMTP_USERNAME/g" docker-compose.yml
    fi

    read -p "What is your SMTP password? " SMTP_PASSWORD
    if [ -n "$SMTP_PASSWORD" ]; then
        sed -i "s/SMTP_PASSWORD=YourEmailPassword/SMTP_PASSWORD=$SMTP_PASSWORD/g" docker-compose.yml
    fi

    read -p "What is your SMTP from email? " SMTP_FROM_EMAIL
    if [ -n "$SMTP_FROM_EMAIL" ]; then
        sed -i "s/SMTP_FROM_EMAIL=YourEmail@gmail.com/SMTP_FROM_EMAIL=$SMTP_FROM_EMAIL/g" docker-compose.yml
    fi

    read -p "What is your SMTP receiver email? " SMTP_RECEIVER_EMAIL
    if [ -n "$SMTP_RECEIVER_EMAIL" ]; then
        sed -i "s/SMTP_RECEIVER_EMAIL=YourEmail@gmail.com/SMTP_RECEIVER_EMAIL=$SMTP_RECEIVER_EMAIL/g" docker-compose.yml
    fi

else
    sed -i "s/SMTP_EMAIL_NOTIFICATIONS_ENABLED=true/SMTP_EMAIL_NOTIFICATIONS_ENABLED=false/g" docker-compose.yml
fi
...
if [ -n "SLACK_FORK "]; then
    ...
    if [[ "$SLACK_NOTIFICATIONS_ENABLED" =~ [Yy] ]]; then
        ...
        if [ -n "$SLACK_WEBHOOK" ]; then
            sed -i "s|SLACK_WEBHOOK=hooks.slack.com/services/|SLACK_WEBHOOK=$SLACK_WEBHOOK|g" docker-compose.yml
        fi

        read -p "What is your Slack channel? " SLACK_CHANNEL
        if [ -n "$SLACK_CHANNEL" ]; then
            sed -i "s/SLACK_CHANNEL=xssalerting/SLACK_CHANNEL=$SLACK_CHANNEL/g" docker-compose.yml
        fi

        read -p "What is your Slack username? " SLACK_USERNAME
        if [ -n "$SLACK_USERNAME" ]; then
            sed -i "s/SLACK_USERNAME=XSS-Hunter/SLACK_USERNAME=$SLACK_USERNAME/g" docker-compose.yml
        fi

        read -p "What is your Slack emoji? " SLACK_EMOJI
        if [ -n "$SLACK_EMOJI" ]; then
            sed -i "s/SLACK_EMOJI=hackerman/SLACK_EMOJI=$SLACK_EMOJI/g" docker-compose.yml
        fi
    else
        sed -i "s/SLACK_NOTIFICATIONS_ENABLED=true/SLACK_NOTIFICATIONS_ENABLED=false/g" docker-compose.yml
    fi
fi
